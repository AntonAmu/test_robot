...
class Bot:
    ...
    def form_data_from_main_page(elements):
        ...
        for element in elements:
            ...
            rows.append(row)
        return elements_for_agencies, Table(rows)
    ...
    def task(self):
        ...
        detail_info = self.get_detail_table(self.parser.parse_element_from_details_page(self.elements_for_agencies))
        self.excel_handler.write_data_to_new_worksheet(detail_info, 'Individual Investments')
        urls_for_downloading = self.parser.url_for_downloading
